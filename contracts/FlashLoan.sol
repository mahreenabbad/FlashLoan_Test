...
contract FlashLoan {
    ...
    address private constant FDUSD =0xc5f0f7b66764F6ec8C8Dff7BA683102295E16409;
    ...
    function initateArbitrage(address _FDUSDBorrow,uint _amount) external{
         IERC20(FDUSD).safeApprove(address(PANCAKE_ROUTER),MAX_INT);
         IERC20(CROX).safeApprove(address(PANCAKE_ROUTER),MAX_INT);
         IERC20(CAKE).safeApprove(address(PANCAKE_ROUTER),MAX_INT);
         
         //liquidity pool of FDUSD and WBNB
         address pair = IUniswapV2Factory(PANCAKE_FACTORY).getPair(
            _FDUSDBorrow,
            WBNB
         );
         ...
         address token1 = IUniswapV2Pair(pair).token1();//FDUSD

         uint amount0Out = _FDUSDBorrow==token0?_amount:0;
         uint amount1Out = _FDUSDBorrow==token1?_amount:0; //FDUSD Amount
         
         bytes memory data = abi.encode(_FDUSDBorrow,_amount,msg.sender);
    ...
    function pancakeCall(
        ...
        (address FDUSDBorrow, uint256 amount, address myAddress) = abi.decode(
            _data,
        ...
        // Place Trades
        uint256 trade1Coin = placeTrade(FDUSD, CROX, loanAmount);
        uint256 trade2Coin = placeTrade(CROX, CAKE, trade1Coin);
        uint256 trade3Coin = placeTrade(CAKE, FDUSD, trade2Coin);
        ...
        // Pay Myself
        IERC20 otherToken = IERC20(FDUSD);
        otherToken.transfer(myAddress, trade3Coin - repayAmount);

        // Pay Loan Back
        IERC20(FDUSDBorrow).transfer(pair, repayAmount);
    }
